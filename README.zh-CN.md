...
相对上游 YouTube Digest 的两项改动：

**1. AI 字幕：没有字幕的视频也能用。** 上游只读 YouTube 原生字幕，遇到关闭了字幕的视频就无能为力。本版本会从视频音频里识别出带时间戳的字幕，时间戳可点击跳转，翻译、概览、笔记等功能照常可用。

**2. 多个 AI 服务商可选。** 上游固定使用 DeepSeek，换模型需要改代码。本版本在设置里直接选：DeepSeek、OpenAI、智谱 GLM、Anthropic Claude、Google Gemini，或任何兼容 OpenAI 格式的接口。

顺带修复了上游一处无障碍问题：按钮白字对比度从 3.81 提升到 5.18，达到 WCAG AA 标准。
...
```
这个视频没有字幕
38 分 56 秒 · $0.03 · 会用掉 Groq 本小时免费额度的 32%
                              [ 生成 AI 字幕 ]
```

点击后从音频生成字幕。实测 39 分钟的视频约 25 秒完成，第一段字幕约 6 秒出现。

### 字幕获取的三层顺序

1. **YouTube 原生字幕** ， 免费、无上限，绝大多数视频走这一层
2. **Supadata** ， 降为备胎，只在第一层取不到时才用，因此免费额度基本用不完
3. **AI 识别** ， 前两层都没有时才提供，且**必须你点击才会开始**

### 费用与限额

语音识别可选 Groq（默认）或 OpenAI Whisper。两家跑的都是 Whisper 模型，区别是谁在托管：

| 服务商 | 每小时音频 | 免费额度 |
| --- | --- | --- |
| Groq | 约 $0.04 | 每小时 7,200 秒、每天 28,800 秒音频 |
| OpenAI Whisper | 约 $0.36 | 无 |

Groq 免费额度的换算（[官方文档](https://console.groq.com/docs/rate-limits)）：

- **单个视频上限 2 小时**，超过则需跨限流窗口分次完成
- 39 分钟的视频：每小时 3 个，每天 12 个
- 1 小时的视频：每小时 2 个，每天 8 个

超过单次上限的视频会**提前告知**，不会跑到一半才失败。中途撞到限流时已完成的部分立即可读，额度恢复后点「继续」接着跑，不重复计费。

生成过的 AI 字幕会缓存，同一个视频再打开不会重复花钱。

设置页有两个开关：**AI 字幕总开关**（默认开）和**自动生成**（默认关，打开后跳过费用确认直接开始）。
